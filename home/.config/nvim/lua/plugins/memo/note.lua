...
return {
  ...
  },
  {
      ...
      desc = "设置编译器 :compiler gcc 输出编译信息到文件：> output.txt 2>&1 用:cfile output.txt 将编译信息加载到 quickfix list 中 或者用:cbuffer output.txt 将当前 buffer 的内容加载到 quickfix list 中。 :copen 打开 quickfix list 窗口。",
  }
}
